...
template<typename MatchState, typename Accum>
struct MultiplierState {
    using state = MatchState;
    using accum = Accum;
};

using empty_state = MultiplierState<S0, literal<0>>;
...
struct Func {
    ...
    using type = MultiplierState<
        new_state,
        literal<In::accum::value + value::value>
...
struct Test {
    ...
    static_assert(std::is_same_v<
        typename fold<Func, empty_state, typename input<
            'm', 'u', 'l', '(', '3', ',','2'
        >::type>::type::state,
        Smul_p_a_c_b<literal<3>, literal<2>>
    >);
